...
for i in range(len(img_names)):
    ...
    xc, corr1, xt, thres_val = frc_util.FRC(sa1, sa2, thresholding=threshold, inscribed_rings=inside_square,
                                            analytical_arc_based=anaRing, info_split=args.info_split)
    _, corr2, _, _ = frc_util.FRC(sb1, sb2, thresholding=threshold, inscribed_rings=inside_square,
                                  analytical_arc_based=anaRing, info_split=args.info_split)
    _, corr3, _, _ = frc_util.FRC(sa1, sb1, thresholding=threshold, inscribed_rings=inside_square,
                                  analytical_arc_based=anaRing, info_split=args.info_split)
    ...
    out_img_name = ('./' + output_dir + '/FRCof_' + out_img_name.split('.')[0] + '.png')
